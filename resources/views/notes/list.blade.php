<x-layout>
    ...
    @foreach($notes as $note)
        ...
        <tr class="text-sm" >
            ...
            <td class="border px-2 "><a href="{{$note->type}}/processnotes/{{$note->site_id}}&target=_blank"><i class="fa-solid fa-pen"></i></a>
            </td></tr>
        @endforeach
    </tbody>
        </table>
        ...
        <table class="table-auto">
            ...
            @foreach($notes as $note)
                ...
                <tr class="text-sm" >
                    ...
                    <td class="border px-2 ">{{ $note->user_id }}</td>
                    <td class="border px-2 ">{{ $note->updated_at->format('dS M, Y') }}</td>
                    <td class="border px-2 "><a href="siteedit?id={{$note->id}}"><i class="fa-solid fa-circle-check"></i></a>
                    </td></tr>
            @endforeach
            </tbody>
        </table>
